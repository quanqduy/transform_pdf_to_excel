...
def func_main_process(input_list):
    ...
    # print(tables)
    '''Add 17.03.2026'''
    po_delivery_date_df = tables[['MA_DON_HANG', 'DELIVERY_DATE']].drop_duplicates().dropna()
    ...
    template_excel = template_excel.merge(po_delivery_date_df, how='left', left_on='Số PO', right_on='MA_DON_HANG')
...
#     # Hàm func_main_process bên dưới của bạn cần đảm bảo đọc được file object
#     final_template = func_main_process(list_pdf)
    
#     # Thay vì lưu file .to_excel tại đây, ta chỉ trả về kết quả cho Streamlit hiện lên web
